            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        GraphNode *node = AddToElementList<GraphNode>(std::stoi(idToken->second), _nodes);
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        GraphEdge *edge = AddToElementList<GraphEdge>(std::stoi(idToken->second), _edges);
...
template <typename T>
T *ChatLogic::AddToElementList(int id, std::vector<T *> &elements)
{
    //check if the current element's ID already exist 
    T *e = nullptr;
    auto isExist = std::find_if(elements.begin(), elements.end(), [&id](T *&e){
        return e->GetID() == id;
    });
